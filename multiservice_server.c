/*
*		 	multiservice_server.c

*	Serveur TCP Multiservice : open a socket for each socket
*	The client must to connect to the port matching the service needed
*
*/
...
int 	passiveTCP(const char *service, int glen);
...
int main(int argc, char *argv[])
{
	...
	struct service  *psv,			/* service table pointer 	*/
			*fd2sv[NOFILE];		/* map fd o service pointer 	*/
	int		fd, nfds;
	fd_set		afds, rfds;		/* readable file descriptor	*/
...
/*-------------------------------------------------------------------
 *
 * reaper - clean up zombie children - only for fork server
 *
 *-------------------------------------------------------------------
 */
